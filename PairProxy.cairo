...
// @title CairoFiSwap V2 Pair Proxy
// @author CairoFi
// @license MIT
...
func constructor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    implementation_hash: felt, token0: felt, token1: felt, proxy_admin: felt
) {
    
    Proxy._set_implementation_hash(implementation_hash);
    
    let calldata: felt* = alloc();
    assert [calldata] = token0;
    assert [calldata + 1] = token1;
    assert [calldata + 2] = proxy_admin;
    library_call(
